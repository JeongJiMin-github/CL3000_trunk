...
//HYP 20060427 str field is ignored in pre-print area(except shop data)
					...
					if(field_num == FIELD_ID_BONUS_POINT) 
					{
						if(PrtStruct.StrBuffer[0]){
							...
							temp_y = PrtStruct.y + Image_Form_logo[1] + Image_Form_bar[1] + image_margin*2;
							if(PrtStruct.y+StrStruct.StrHeight + image_margin*2 > temp_y) temp_y = PrtStruct.y+StrStruct.StrHeight + image_margin*2;

							// Print Outline
							PrtRect(PrtStruct.x,PrtStruct.y,PrtStruct.x+StrStruct.StrWidth,temp_y ,3,BLACK);

							PrtStruct.x +=	image_margin;
							PrtStruct.y +=	image_margin;
							backup_x = PrtStruct.x;
							backup_y = PrtStruct.y;

							if(StrStruct.StrHeight < Image_Form_logo[1] + Image_Form_bar[1])StrStruct.StrHeight = Image_Form_logo[1] + Image_Form_bar[1];

							if(PrtStruct.StrBuffer[0] == '1') // Print Logo and Bar
							{
								PrtStruct.y +=	(temp_y - PrtStruct.y - image_margin*2)/2;
								PrtStruct.y -=	(Image_Form_logo[1] + Image_Form_bar[1])/2;
								// Print LOGO
								BitmapOut(0x21);

								PrtStruct.x +=	(Image_Form_logo[0]-Image_Form_bar[0])/2;
								PrtStruct.y +=	Image_Form_logo[1];

								// Print Bar								
								reverse_percent = (PrtStruct.StrBuffer[3]-'0') * 100 + (PrtStruct.StrBuffer[4]-'0') * 10 +(PrtStruct.StrBuffer[5]-'0') * 1;
								BitmapOut(0x22);
								
								PrtStruct.x = backup_x;
								PrtStruct.y = backup_y;
								PrtStruct.x +=	Image_Form_logo[0];
								StrStruct.StrWidth -=	Image_Form_logo[0];
							}
							else if(PrtStruct.StrBuffer[1] == '1') // Print Bar
							{
								PrtStruct.x +=	(Image_Form_logo[0]-Image_Form_bar[0])/2;
								PrtStruct.y +=	(temp_y - PrtStruct.y - image_margin*2)/2;
								PrtStruct.y -=	Image_Form_bar[1]/2;
							...
							StrStruct.StrWidth -= image_margin*2;
							StrStruct.StrHeight-= image_margin*2;
							// Print Text								
							StrStruct.Magx = backup_magx;
							StrStruct.Magy = backup_magy;
