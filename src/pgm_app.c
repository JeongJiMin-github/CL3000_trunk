...
INT8U Label_name_Scroll_flag = OFF;
INT8U Label_name_buf[64];
#endif
...
//		strcat((char *)buffer, " ");
   ...
   #endif
 #else
		Dsp_Write_String(disp_p,(HUGEDATA char *)buffer+9); // "원산지 : " 생략
 #endif
#else
		display_lcd_clear();
		Dsp_Write_String(disp_p,(HUGEDATA char *)buffer);
...
#ifdef USE_LABEL_NAME_SCROLL
void plucall_label_id_scroll(long num)
{	
	POINT disp_p;
	INT16U scrollLen;
	INT16S x_offset;
	...
	scrollLen = strlen(Label_name_buf);
	...
	else if (ParamScroll_Xpt < SCRL_BEGIN_TIME + scrollLen + SCRL_BEGIN_TIME) x_offset = scrollLen;
	else x_offset = 0;

	font_width = display_font_get_width();
	disp_p = point(0, DSP_SCROLL_MSG_X - x_offset * font_width);
  #ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
    Dsp_Write_Reverse_String(disp_p,(HUGEDATA char *)Label_name_buf);
  #else
	Dsp_Write_String(disp_p, (HUGEDATA char *)Label_name_buf);
  #endif
...
//		strcat((char *)buffer, " ");
	...
	if(result == FALSE) {	
        ...
        Dsp_Write_Reverse_String(disp_p,(HUGEDATA char *)cap.form);
	#ifdef USE_LABEL_NAME_SCROLL
		memcpy(buffer, cap.form, sizeof(cap.form));
	#endif
  #else
		Dsp_Write_String(disp_p,(HUGEDATA char *)cap.form);
		return;	
  ...
  #else
	Label_name_Scroll_flag = ON;
  #endif
	memset(Label_name_buf, 0, sizeof(Label_name_buf));
	memcpy(Label_name_buf, buffer, sizeof(buffer));
#endif
}
