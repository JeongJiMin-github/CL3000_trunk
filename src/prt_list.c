...
//	INT8U stype;
	...
	base_addr = DFLASH_BASE + FLASH_DISCOUNT_AREA;
...
//	p_type_offset = plu_get_ptype_point(4, &stype, &usize);	//plu type offset
	...
	for(cur_id = 0; cur_id < num && !flagStopPrint; cur_id++){
		...
		//if(dc_no) {
			addr = base_addr + (INT32U)(dc_no)*DISCOUNT_STRUCT_SIZE;
...
//			plu_addr = plu_table_search(dc_str.dept_no, dc_str.plu_no);
			...
			memcpy((INT8U*)&string_buf,(INT8U *)cap_addr_dayweek, 32);
			string_buf[32] = 0;
			PutDataRxBuf(&CommBufPrt,string_buf,strlen(string_buf));
			RcptCmd_CharTab(7);
			...
			memcpy((INT8U*)&string_buf,(INT8U *)cap_addr_s_date, 32);
			string_buf[32] = 0;
			PutDataRxBuf(&CommBufPrt,string_buf,strlen(string_buf));
			RcptCmd_CharTab(7);
			...
			memcpy((INT8U*)&string_buf,(INT8U *)cap_addr_e_date, 32);
			string_buf[32] = 0;
			PutDataRxBuf(&CommBufPrt,string_buf,strlen(string_buf));
			RcptCmd_CharTab(7);
			//sprintf(string_buf, "%s", dc_str.start_month);
...
//sprintf(string_buf, "Start Time");
			memcpy((INT8U*)&string_buf,(INT8U *)cap_addr_s_time, 32);
			string_buf[32] = 0;
			PutDataRxBuf(&CommBufPrt,string_buf,strlen(string_buf));
			RcptCmd_CharTab(7);
			//sprintf(string_buf, "%s", dc_str.start_month);
...
//sprintf(string_buf, "End Time");
			memcpy((INT8U*)&string_buf,(INT8U *)cap_addr_e_time, 32);
			string_buf[32] = 0;
			PutDataRxBuf(&CommBufPrt,string_buf,strlen(string_buf));
			RcptCmd_CharTab(7);
			//sprintf(string_buf, "%s", dc_str.start_month);
...
INT8U scroll_print_list(INT16U start_id, INT16U end_id)
{
	...
	cap_addr_text       = caption_search_by_code(0x2704) + 2;
	cap_addr_stype      = caption_search_by_code(0x2705) + 2;
	cap_addr_stype_num  = caption_search_by_code(0x2706) + 2;
	cap_addr_usemsg     = caption_search_by_code(0x2709) + 2;
	cap_addr_usedate    = caption_search_by_code(0x2710) + 2;
	cap_addr_week       = caption_search_by_code(0x1c14) + 2;
	cap_addr_start_date = caption_search_by_code(0x1c15) + 2;
	cap_addr_end_date   = caption_search_by_code(0x1c17) + 2;
	cap_addr_start_time = caption_search_by_code(0x1c16) + 2;
	cap_addr_end_time   = caption_search_by_code(0x1c18) + 2;
	...
	for(cur_id = start_id; cur_id <= end_id && !flagStopPrint; cur_id++){
		...
		if(scroll.stype > 2) {
			...
			//prtlist_captitle_adjust(string_buf, cap_addr_stype_num+(INT32U)scroll.stype*CAP_MSG_LENGTH);
			//memcpy(string_buf, (char *)(cap_addr_stype_num+(INT32U)scroll.stype*CAP_MSG_LENGTH), 32);
			RcptCmd_PrintMultiLine((char *)(cap_addr_stype_num+(INT32U)scroll.stype*CAP_MSG_LENGTH), 32, TICKET_MARGIN+TICKET_TXT_MARGIN+7);
		}
				  
		RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
		prtlist_captitle_adjust(string_buf, cap_addr_usemsg);
		PutDataRxBuf(&CommBufPrt,string_buf,strlen(string_buf));
		RcptCmd_CharTab(7);
		...
		RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
		prtlist_captitle_adjust(string_buf, cap_addr_usedate);
		PutDataRxBuf(&CommBufPrt,string_buf,strlen(string_buf));
		RcptCmd_CharTab(7);
