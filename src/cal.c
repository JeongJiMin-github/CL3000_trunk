...
void CalRun_KeyTest(void)
{
	...
	while(1) {
		...
		{
			caption_split_by_code(0x97a1, &cap,0);
		}
		caption_adjust(&cap, NULL);
  ...
  #ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
		disp_p = point(0,0);
		Dsp_Write_String(disp_p,(INT8U *)form_graphic);
  #endif
...
INT8U UseBootCountModeForDebug = 0;
#endif
...
//	if (country_str[0]) strcat(country_str, " ");	// country_str이 있을 때, 스페이스 추가
  ...
  #ifdef USE_ADM_AUTO_RESTORE
    end_id = 10; // init_err_cnt, eep_err_cnt;
  #else
	end_id = 8;
  #endif
#else
			...
			switch(cur_id) {
				case 0 :	// Main
					...
					else
					{
  					...
  					if (country_str[0])
					{
						sprintf(ver_str, "%s%c.%c%c.%s%s", country_str, fversion.version, fversion.reversion[0], fversion.reversion[1]
									     , subversion, dealer_str);
					}
					else
					{
					...
					sprintf(ver_str, "%8d", bootCntForWhiteScreen);
					sprintf(des_str, "Boot.Cnt");
#else
					if (calc_checksum)
					{
  #ifndef DISPLAY_REVISION_INFO
...
void CalRun_Scale_Info(void)
{
	...
	while(1) {
		nth = 0;
		page_first_id = cur_id;
		memset(unit_str,0x00,20);
		memset(total_str,0x00,20);
		while(1) {
			switch(cur_id) {
				case 0 : //Print Lenght(SUM)
					sprintf(unit_str, "TTL LEN");
					sprintf(total_str, "%8ld", TphUseData.TotalCnt);
					break;
				case 1 : //Print Lenght
					sprintf(unit_str, "RPL LEN");
					sprintf(total_str, "%8ld",  TphUseData.AfterCnt);
					break;
				case 2 : // Inspection Date
					sprintf(unit_str, "DATE");
					...
					temp_date %= 100L;
					day = temp_date;                    
					sprintf(total_str,"20%02d.%02d.%02d",year ,month ,day);
					break;
				case 3 : // Overload Count(SUM)
					sprintf(unit_str, "TTL OL");
					if (set_val[0] == -1) {
						sprintf(total_str, "DisCont");
					}
					else	{
						sprintf(total_str, "%8ld", set_val[0]);
					}
					break;
				case 4 : // Overload Count
					sprintf(unit_str, "OL CAL");
					if (set_val[1] == -1) {
						sprintf(total_str, "DisCont");
					}
					else	{
						sprintf(total_str, "%8ld", set_val[1]);
					}
					break;
				case 5 : // Inspection Date
				case 8:
					sprintf(unit_str, "CALDATE");
					if (set_val[2] == -1) {
						sprintf(total_str, "DisCont");
					}
					else	{
				...
				case 6 : // Weighing Count(SUM)
					sprintf(unit_str, "Total Weighing Frequency");
					if (set_val[3] == -1) {
						sprintf(total_str, "DisCont");
					}
					else	{
						sprintf(total_str, "%8ld", set_val[3]);
					}
					break;
				case 7 : // Weighing Count
					sprintf(unit_str, "W.F CAL");
					if (set_val[4] == -1) {
						sprintf(total_str, "DisCont");
					}
					else	{
						sprintf(total_str, "%8ld", set_val[4]);
					}
					break;
				case 9 : // Using Hour(SUM)
					sprintf(unit_str, "OP.TIME");
					//OperTime.TotalCnt = get_nvram_lparam(NVRAM_MORNITORING_USING_TIME);
					sprintf(total_str, "%8ld", OperTime.TotalCnt);
					break;
				case 10 : // Using Hour
					sprintf(unit_str, "TIME.Ins");
					sprintf(total_str, "%8ld", OperTime.AfterCnt);
					break;
				case 11 : // Inspection Date
					sprintf(unit_str, "Ins.Date");
			...
			display_clear(DISPLAY_UNITPRICE);
			display_string(DISPLAY_UNITPRICE,(INT8U *)unit_str);
			display_clear(DISPLAY_PRICE);
			display_string_pos(DISPLAY_START_POS_PRICE, (INT8U*)total_str);
			...
			case KEY_TYPE_FUNCTION :
				switch(KeyDrv.CnvCode) {
					...
					case KP_ZERO : // Erase Count
						...
						if(!rtc_Batt_Err) {
							switch (page_first_id) {
								case 0://Print Lenght(SUM)
								//case 1://Print Lenght
								//case 2://Inspection Date
									if (permission_check_sub(PMS_DEALER,0)) {
										sprintf(str, "Wait for Clr!");
										disp_p = point( Startup.menu_type*Startup.menu_ygap, 0);
										Dsp_Write_String(disp_p,(HUGEDATA char*)str);
										Dsp_Diffuse();
								...
								//case 6: // Weighing Count(SUM)
								//case 7: // Weighing Count
								//case 8: // Inspection Date
									if (status_run.run_mode == RUN_CALIBRATION) {
										sprintf(str, "[Print]Erase");
										disp_p = point( Startup.menu_type*Startup.menu_ygap, 0);
										Dsp_Write_String(disp_p,(HUGEDATA char*)str);
										Dsp_Diffuse();
										while(!KEY_Read());
...
void CalRun_MemoryInformation(void)
{
	...
	INT8U i;
	INT16U input_x;
	INT16U cap_code[2][3] = {{0x1866, 0x97aa, 0x97ab},	// Prog. Mode 
				 {0x8560, 0x9560, 0x9561}};	// Cal Mode
	INT8U cap_id;
	...
	while(1) {
		...
		if (Startup.menu_type != 0) {
			menu_display_menucode(CurMenuCode, 1);
			menu_displaytitle(cap_code[cap_id][0], 0);//0x8560,0);
			menu_writepage(1, 1);
			disp_p = point((Startup.menu_type+1)*Startup.menu_ygap, 0);
			caption_split_by_code(cap_code[cap_id][1], &cap, 0);//0x9560, &cap,0);	// "Check Flash Chip"
		...
		caption_split_by_code(cap_code[cap_id][2], &cap, 0);//0x9561, &cap,0);	// "Total Flash Size :"
...
void CalRun_ScaleTypee_callback(long type32)	//Added by JJANG 20070621
{
	...
	char typename[2][14]={"STANDARD TYPE","POLE TYPE"};
#else
	...
	type = (INT8U)type32;
#if defined(USE_CL5200_KEY) || defined(USE_CL3000_KEY)
	if (type>0 && type<3) strcpy(cap.form,typename[type-1]);
...
#if defined(USE_CL5200_KEY) || defined(USE_CL3000_KEY)
	cap.input_max = 2;	//scale type max 2
#endif 
...
void CalRun_SetCreep(void)
{
	...
	if(result == MENU_RET_SAVE) {
		...
		cmd_param += ADM_SUB_CREEP_ON;//"01"
		ADM_WaitData(ADM_CMD_SET_PARAMETER, cmd_param, ADM_PARAM_ON);
		if(ADM_Status.Version >= 101) {
			cmd_param = set_val[0];
			cmd_param += ADM_SUB_CREEP_NUM;//"02"
			ADM_WaitData(ADM_CMD_SET_PARAMETER, cmd_param, ADM_PARAM_ON);
		}
		ADM_WaitData(ADM_CMD_SAVE_SETS, 83419, ADM_PARAM_ON);				// Save Settings
		Cal_flagAdmBackup = 1;
