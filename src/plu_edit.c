...
//DEL071023	clear_temp_inhibit_list();
	...
	for (cur_id = 0; cur_id < maxid; cur_id++)
	{
            ...
            case PTYPE_FIXED_WEIGHT:
				...
				else
				{
...
#elif defined (USE_FIXEDWEIGHT_UNIT_OZ)
					if(GlbOper.use_oz_fixedweight) // para_754 Use oz in Fixed Weight Unit	1: Fixed Weight 의 단위를 oz로 사용 / 0: Fixed Weight 의 단위를 저울의 현 설정값을 따라가도록 설정
						dpoint = 0; //oz decimal point -> 소수점 사용 안함 
					else
						dpoint = get_decimal_pt(6, 0);
#else
					dpoint = get_decimal_pt(6, 0);
		...
		switch (stype) {
			...
			case 'I':
#ifdef USE_AUSTRALIA_COOL_FUNCTION
				Menu_InputCreate(ptype, ( char *)str, MENU_TYPE_STR, sl+2, dlength, DEFAULT_INGREDIENT_TEXT_ALL_SIZE,
								maxvalue, minvalue, 0, NULL, ( INT8U*)data_point, NULL, NULL);						
...
#ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
        caption_form_split_menu_graphic(string_buf);
#endif
...
#endif
#ifdef USE_LOGISTICS_PLU	//1144에서 reserved 안보이도록 처리
