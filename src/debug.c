...
void ad_firmware_download(void)	// Added by CJK 20050614
{
	...
	ADM_ScanBaudrate(BAUD_RATE_57600, BAUD_RATE_38400);
...
void  debug_Factory(void)
{
	...
	while (1) {
		...
		ret1=0;
	    	switch (test_mode) {
	    	    case 0x01: sprintf(gmsg,"R%1X\x0a",test_mode);
	    		       send      = 1;
	    		       test_mode=0x52;
	    		       break;
	    	    case 0x02: if (disp_first==0) {
	    				disp_first   = 1;
				...
				if ((INT16U)(SysTimer100ms-disp_timeout)>4) {
	    			...
	    			break;
	    	    case 0x03:  CalRun_InitADM(2, OFF);// Modified by CJK 20050816
	    	    		ret=(INT16S)ADM_Init();				
	    	    		test_ok[test_mode]=1;
	    			if (ret) {
	    				cc='E';
	    			...
	    			send=1;
	    			goto OUT_END1;
	    	    case 0x04:  disp_timeout=SysTimer100ms;
				while (1) {
...
//					FlashNVRAMStruct.buffer[ret]=(INT8U)ret;
//				}
//				test_ok[test_mode]=1;
//				for (ret=0; ret<10; ret++) {
//					if (FlashNVRAMStruct.buffer[ret]!=(INT8U)ret) {
//						test_ok[test_mode]=0;
//						cc='E';
//						break;
//					}
...
//					ret=FlashCheckMem();
//	    			if (test_mode==0x06) {
//	    				if ((ret&0x30)==0x30) ret1=1;
//	    			} else {
//	    				if ((ret&0xf0)==0xf0) ret1=1;
//	    			}
//	    			if (!ret1) cc='E';
//				test_ok[test_mode]=(INT8U)ret1;
	    			goto OUT_END1;
	    	    case 0x07:  ret=(INT16S)EthInit();
	    			if (!ret) {
	    				cc='E';
					ret=0;
				} else ret=1;
	    	   ...
	    	   case 0x09:	CASH_ON;
	    	    ...
	    	    case 0x0A:  if (!disp_first) {
				   ...
				   ret=PS2_InKey();
	    			   BuzOn(1);
				   if ((ret==0x00B3) || (ret==0x0014)) { // Enter or ESC Key
					test_ok[test_mode]=1;
					goto OUT_END1;
				   } else {
		    			   key_count++;
		       		      	   sprintf(gmsg,"P%04X\x0a",ret);
					   send=1;
				   }
	    	   	      	}
	    		      	break;
	    	    case 0x0B:  printer_sensor_keytest((INT16U *)&gab,(INT16U *)&peel,3);
	    			sprintf(gmsg,"S%04x.%04x\x0a",gab,peel);
	    		        goto OUT_END2;
	    	    case 0x0C: //
	    			PrtSensorFlagInit();
