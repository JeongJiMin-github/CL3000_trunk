...
extern void barcodefile_default_for_india(void);
#endif
...
extern void feedLength_default_for_indiaTATA(void);
#endif
...
#elif defined(USE_CHN_EMART_DEFAULT)
...
#endif
	set_global_bparam(GLOBAL_DISPLAY_PRICEPOS	,17);
...
void set_clerk_init(void)
{
	...
	sprintf((char *)clerk.password	 ,"%s","4520");
#else
...
void global_set_country(INT16U country, INT8U flagKeepDefaultCaption)
{
	...
	global_set_default_parameter(DISPLAY_PRICE_DIGIT, flagKeepDefaultCaption);
...
#endif    
#ifdef USE_TRACE_STANDALONE 
	set_global_bparam(GLOBAL_SALE_SETUP_FLAG15, 0x01); // Default " 상품 개체 "
...
void plu_inhibit_default_eut(void)
{
  ...
  #ifdef USE_STANDARD_DIRECT_INGREDIENT
	plu_inhibit_bycode(PTYPE_INGREDIENT_TEXT,0);	//ingredient text
  #endif

  #ifdef USE_INDIA_DEFAULT
	plu_inhibit_bycode(PTYPE_EXT_BARCODE, 0); //Ext. Barcode // 인도 요청 기본값 변경 (Reliance 납품 건) Use Ext.Barcode as ItemCode(M1873) 값 할성화에 따른 Ext Barcode 필드 할성화
  #endif

  #ifdef USE_INDIA_TATA_DEFAULT
	plu_inhibit_bycode(PTYPE_EXT_BARCODE, 0); //Ext. Barcode // 인도 요청 기본값 변경 (Reliance 납품 건) Use Ext.Barcode as ItemCode(M1873) 값 할성화에 따른 Ext Barcode 필드 할성화 -> TATA Group 도 Reliance와 동일한 사양 
  #endif
#endif
}
...
//	plu_inhibit_bycode(PTYPE_NETWEIGHT_DECIMAL,0);	
...
#else 
void plu_inhibit_default_un_etc()
{
	plu_inhibit_bycode(PTYPE_ALL,1);			//Disable All
	plu_inhibit_bycode(PTYPE_GROUP_NO,0);		//Group
...
void set_barcode_default_kr(void)
{
...
#elif defined(USE_GSMART_DEFAULT)
	barcodefile_default_for_GSmart();
#elif defined(USE_NHMART_DEFAULT) || defined(USE_NHMART_SAFE_DEFAULT)
	...
	for (i=0; i<9; i++) set_global_bparam(GLOBAL_BARCODE_STD_WID + i, 3);
#elif defined(USE_HYUNDAI_DEFAULT)
...
#if !defined (USE_GSMART_PLU) || !defined (USE_LOTTEMART_DEFAULT)
...
#if defined(USE_USA_INGREDIENT) || defined(USE_NORTH_AMERICA_INGREDIENT)
	ret+=plu_inhibit_bycode(PTYPE_NO_OF_LINK_PLU, 0);
