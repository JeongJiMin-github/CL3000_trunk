...
void NumericValue(INT32U maxValue, INT32U *value)
{
	...
	if (KeyDrv.CnvCode > 9) 
	{
		status_scale.cur_keyvalue = status_scale.cur_keyvalue * KeyDrv.CnvCode;
	} 
	else 
	{
		status_scale.cur_keyvalue = status_scale.cur_keyvalue * 10 + KeyDrv.CnvCode;
	}
	status_scale.cur_keyvalue %= maxValue;
...
void key_numeric(void)
{
	...
	if(!status_scale.ride_max && !InputQuickPluState)//(!Operation.override && status_scale.cur_pluid)
	{
		...
		if ( !(ret&0x08) )
		{
            ...
            return;		//광고문키 사용시 광고 1~9까지만 사용하도록 제한함.
#else
			return;
#endif
		}
	...
	RUN_FUNC:
			if(processFlag) {
				switch(KeyDrv.CnvCode) {
					...
					case KS_ZERO  : keyapp_zero(); break;
					case KS_TARE  : 
						...
						break;
					case KS_CLEAR    : keyapp_clear(); break;
					case KS_MODE     : 
...
void	SaleInsertLabelData(void)
{
	...
	if(KorTrace_Flag.flagPrint==2) {	// Added by CJK 20060316
		...
		LabelData.Price.UnitPrice[PRICE_ORIGIN] = KorTrace_ReceiveCmd.unitprice[PRICE_ORIGIN];
		LabelData.Price.UnitPrice[PRICE_RESULT] = KorTrace_ReceiveCmd.unitprice[PRICE_RESULT-1];
		LabelData.Price.UnitPrice[PRICE_SAVED] = LabelData.Price.UnitPrice[PRICE_ORIGIN] - LabelData.Price.UnitPrice[PRICE_RESULT];

		LabelData.Price.TotalPrice[PRICE_ORIGIN] = KorTrace_ReceiveCmd.totalprice[PRICE_ORIGIN];
		LabelData.Price.TotalPrice[PRICE_RESULT] = KorTrace_ReceiveCmd.totalprice[PRICE_RESULT-1];
		LabelData.Price.TotalPrice[PRICE_SAVED] = LabelData.Price.TotalPrice[PRICE_ORIGIN] - LabelData.Price.TotalPrice[PRICE_RESULT];

		LabelData.discount_rate = KorTrace_ReceiveCmd.discount_rate;	//저울나라 할인율
		
		LabelData.Price.DisplayedPrice = KorTrace_ReceiveCmd.totalprice[2];
		LabelData.SellbyDate = (INT32U) KorTrace_ReceiveCmd.sellbydate;  //SG060322
		KorTrace_ReceiveCmd.sellbydate = 0;  //SG060323	 1회 Print용

		LabelData.manufacture_date = KorTrace_ReceiveCmd.packedondate;		//저울나라 제조일자(yymmdd)
...
void keyapp_call_department(INT16U dept_id,INT8U fg)
{
	...
	Menu_InputCreate(1, (HUGEDATA char *)cap.form, MENU_TYPE_YN, cap.input_x, cap.input_dlength, cap.input_length,
			cap.input_max, cap.input_min, 0, &ret_value, NULL, NULL, NULL);
...
void PLU_external_print(INT8U sellbydate_check, INT8U prepack_type)
{
	...
	if(CurTraceStatus.gradeNo || (TraceMeatGradeListNum[CurTraceStatus.gradeNo - 1] != 0))
	{
		...
		sprintf(grade_list, "%s", &TraceMeatGradeListText[v8_1][0]);
	}
	else
	{
		sprintf(grade_list," ");
	...
	{
		v8_1 = 0;
		for (v8_2 = 0; v8_2 < CurTraceStatus.gradeNo - 1; v8_2++)
		{
			grade_mark[v8_1++] = ' ';
			grade_mark[v8_1++] = ' ';
		}
