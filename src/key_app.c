...
INT8U Cvt_VietnamPreview(INT8U *str, INT16U leng, INT8U cur_pt, INT8U mode)
{
	INT16U i;
	INT8U ret;
		...
		INT16U leng, INT16U win_size, INT8U y, INT16U x, INT16U properties, INT8U use_sec, INT8U pass)
{
	...
	static INT16U StartTimer_100ms;
	StartTimer_100ms = SysTimer100ms;
#endif

#ifndef USE_MENU_DISPLAY_ON_GRAPHIC_AREA	
	...
	if (dsp_mode == KI_GRAPHIC) {
        ...
        while((SysTimer100ms - prevDispTimer100ms) <= MENU_DSIP_TIME) // KEY IN STRING has no callback func.
        {
            if(KEY_Read())break;
        }
        disp_p = point(0, 0);
        memset((INT8U *)&tmp_str, 0x20, 12);
        Dsp_Write_String(disp_p, (HUGEDATA char *)tmp_str);
        tmp_str[0]=0;
  #endif
	}
#endif
	...
	while(1) {
		...
		if (cur_pt == 0 || input_mode == KI_IP_MODE) { //처음 위치에 있을 때 or IP input mode
			...
			if (use_sec == 1) {
				...
				if(cursor_direct == 1) {
					...
					else cursor.chw.byte.right = buffer[cur_pt]; //마지막 글자표시
		...
		if (convert_cur_pt == 0) { //처음 위치에 있을 때 or IP input mode
			if(cursor_direct == 1) {
				cursor.chw.byte.left = arab_codetemp[convert_cur_pt];
				cursor.chw.byte.right = arab_codetemp[convert_cur_pt+1];
			} else if(cursor_direct == 2) {
				cursor.chw.byte.left = arab_codetemp[convert_cur_pt];
				cursor.chw.byte.right = arab_codetemp[convert_cur_pt+1];
			}
			...
			if(cursor_direct == 1) {
				cursor.chw.byte.left = arab_codetemp[convert_cur_pt]; //다음 문자(커서 다음 위치)
				cursor.chw.byte.right = arab_codetemp[convert_cur_pt+1];	//현재 문자 (커서 위치)
			} else if(cursor_direct == 2) {
				cursor.chw.byte.left = arab_codetemp[convert_cur_pt];
				cursor.chw.byte.right = arab_codetemp[convert_cur_pt+1];
			}
			...
			if (use_sec == 1) {
				...
				if(cursor_direct == 1) {
					cursor.chw.byte.left = arab_codetemp[convert_cur_pt]; //다음 글자 표시 (변환 이전 폰트 표시)	 
					if(cur_pt==0) cursor.chw.byte.right = 0x20;
					else cursor.chw.byte.right = arab_codetemp[convert_cur_pt-1]; //마지막 글자표시
				} else if(cursor_direct == 2) {
					cursor.chw.byte.left = arab_codetemp[convert_cur_pt]; //마지막 글자표시				
					cursor.chw.byte.right = arab_codetemp[convert_cur_pt+1];	//다음 글자 표시			
				}
			}				
		}
#elif defined(HEBREW_FONT)
		...
		if (convert_cur_pt == 0) { //처음 위치에 있을 때 or IP input mode
	...
	#endif	//USE_ARAB_FONT || USE_ENG_BASED_ARAB_SUPPORT
  #endif		//USE_VIETNAM_FONT
#endif	//USE_ARAB_CONVERT
			...
			while(!KEY_Read()){
				...
				cursor_blink(&cursor);
#ifdef USE_REALTIME_DATA_EVROOPT
				if (((INT16U)(SysTimer100ms - StartTimer_100ms) >= 100)&& MenuFlag)
				{
					StartTimer_100ms = SysTimer100ms;
					return KP_ESC;
				}
  ...
  #ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
                if (menuDispOn && ((INT16U)(SysTimer100ms - prevDispTimer100ms) >= MENU_DSIP_TIME))// display on while 1 sec 
                {
                    menuDispOn = OFF;
                    if (keyin_callback_on) (*keyin_callback)(temp_val);
                }
...
#define KS_SPECIALUSE_OFFSET	400	// 용도키 401~500
	...
	INT16U  keycode[]={								  // Function Key Code
		...
		KS_OFFLINE		,KS_CALL_INDIV		,KS_UNUSE		, // 120..	//(121)..
		KS_UNUSE		,						  // 123..	//(124)..
#else
#ifndef USE_ONLY_LABEL
		KS_CLERK_33		,						  // 92..	//(93)..
		...
		KS_UNUSE		,KS_UNUSE		,KS_FIXED_5PER	, //230.. //(231)..
...
PAGE_VIEW_01:
  ...
  #ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
	sprintf(string_buf, "%s:%s", form_graphic, keylocation[i]);
  #else
...
PAGE_KI_IN:
	...
	else if(enter_count == 1)
	{
		...
		result = keyin_yesno(KI_DEFAULT, &input_value, 
...
{
	if((ch >= 0x41 && ch <= 0x5a) || (ch >= 0x61 && ch <= 0x7a)) return 1;
	else if(ch >= 0x30 && ch <= 0x39) return 2;
	else if(ch>=0x23 && ch<=0x25) return 3;
	else return 0;
}
...
{
	int    i,j,k_temp,e_temp, bracketStart, bracketEnd;
...
HEB:		if(HebCheck((INT8U)cur_ch))
			...
			{  //히브리어 뒤 공백, 특수문자는 히브리어로 처리
				...
				if(cur_ch==0x2f || cur_ch==0x2e) slashNum++;	//별도 특수 문자(/,.) 처리 
				else slashNum = 0;
				if(EngCheck(cur_ch)==3) exceptNum++;  //별도 특수 문자(#,$,%) 처리 
...
ENG:		k_temp=hebrew_codetemp_pnt;
			...
			if (HebCheck((INT8U)cur_ch) || (hBracketOpen && BracketCheck(cur_ch) && BracketCloseChk(hBracketOpen,cur_ch)))  //영문 + 히브리어 경우 -> 영문 뒤집어 저장 (영문 뒤 특수문자, 공백은 영문으로 처리)
			{
				if (hebrew_nengtemp || hebrew_nnumtemp) 
				{
				...
				goto HEB;				
					...
					(EngCheck(cur_ch)==3 && EngCheck(hebrew_engtemp[heb_nengtottemp-1])==2)||	//현재 문자 '예외문자' 이고  바로 전 문자 '숫자'
					(eBracketOpen && (cur_ch != 0x20) && hebrew_nengtemp)) 	//'괄호열기'상태이고 현재 문자 '공백X'이고 '영문' 있는 경우
			{
				
				if(specChgNum || specSepNum)		//특수기호 포함된 경우
				{
					if(specSepNum && !exceptNum && slashNum!=1 && !eBracketOpen && !engChgEnable/*&& !hBracketOpen*/)
					{
						...
						if(hebrew_nnumtemp && (EngCheck(cur_ch)==1 || EngCheck(cur_ch)==2))
						{
					...
					{	//특수기호 뒤 숫자나 영문 올 경우
						if(specSepNum && ((exceptNum && EngCheck(cur_ch)==2) ||	//이전 문자 '%,$,#' 이고 숫자 입력인 경우
							(slashNum==1 && EngCheck(cur_ch)==2 && (hebrew_codetemp[k_temp]==0x2f || hebrew_codetemp[k_temp]==0x2e))))	//바로 이전 문자 '/' 이고 숫자 입력인 경우
						{ 
							enSpectemp = specSepNum;
						} 
						else if	(specChgNum && (hebrew_nengtemp && !eBracketOpen && !bracketClose && engChgEnable/*&& !slashNum && !exceptNum*/))	//이전 문자 '영문' 이고 '괄호 열기' 안 한 경우
						{
						...
						if(enSpectemp)
						{
							...
							for(i=enSpectemp; i>0; i--)
							{
								...
								if(EngCheck(hebrew_engtemp[heb_nengtottemp])==1) hebrew_nengtemp++;
								else if(EngCheck(hebrew_engtemp[heb_nengtottemp])==2) hebrew_nnumtemp++;
						...
						else if(hebrew_nengtemp && bracketClose) 	//영문방향 괄호 닫기 이후 영문 입력 시 처리
						{
							...
							for(i=k_temp+1; i>0; i--)
							{
								...
								if(EngCheck(hebrew_engtemp[k_temp])==1) hebrew_nengtemp++;
								else if(EngCheck(hebrew_engtemp[k_temp])==2) hebrew_nnumtemp++;
				...
				//if(BracketCheck(cur_ch) && eBracketOpen && eBracketOpen == cur_ch)
				{
					...
					for(i=0; i<heb_nengtottemp; i++) 
					{
						if(bracketStart) 
						{
							if(EngCheck(hebrew_engtemp[i])==1) { bracketStart=0; break; }		//조건 추가 필요 (괄호 안 히브리어 + 숫자 + 영문 경우 ) 
						}
						if(hebrew_engtemp[i]==BracketCloseConv(eBracketOpen)) bracketStart=i;
					}
					...
					//if(hebrew_nengtemp && EngCheck(hebrew_engtemp[heb_nengtottemp-1])==2) {
						...
						for(i=e_temp; i>0; i--) 
						{
							if(i>bracketStart) 
							{
								...
								if(EngCheck(hebrew_engtemp[bracketStart+j])==1) hebrew_nengtemp--;
								else if(EngCheck(hebrew_engtemp[bracketStart+j])==2) hebrew_nnumtemp--;
								//else if(EngCheck(hebrew_engtemp[bracketStart+j])==3) hebrew_nengtemp--;	//별도 특수 문자(#,$,%) 처리 
							...
							{
								if(SpecialCheck(hebrew_engtemp[i])) 
								{
									hebrew_codetemp[k_temp--]=BracketOpenConv(hebrew_engtemp[i]);
									if(EngCheck(hebrew_engtemp[i])==1) hebrew_nengtemp--;
									else if(EngCheck(hebrew_engtemp[i])==2) hebrew_nnumtemp--;
									//else if(EngCheck(hebrew_engtemp[i])==3) hebrew_nengtemp--;	//별도 특수 문자(#,$,%) 처리 
									else if(SpecialCheck(hebrew_engtemp[i])) hebrew_nengspectemp--;	//별도 특수 문자((,),[,],{,}) 처리 
									heb_nengtottemp=hebrew_nengtemp+hebrew_nnumtemp+hebrew_nengspectemp;
									hebrew_engtemp[i] = 0;
								} else break;
							}
				...
				if(EngCheck(cur_ch)==1) hebrew_nengtemp++;	//영문 문자 처리
				else if(EngCheck(cur_ch)==2) hebrew_nnumtemp++;	//숫자 처리
				//else if(EngCheck(cur_ch)==3) hebrew_nengtemp++;	//별도 특수 문자(#,$,%) 처리 
...
SPEC:			if(cur_ch!=0x20) cursor_direct = 1; //입력 안 된 공간은 빈 문자로 처리되므로 다른 특수기호만 방향 지정
				...
				if(hebrew_nnumtemp) numChgEnable = 1;
...
INT16U Convert_HebrewPreview(INT8U *str, INT16U leng, INT16U cur_pt, INT8U firstInputFlag)
{
	...
	INT8U arablist[] = {
		...
		/*A0*/	0xA0,
		/*B0*/							
		/*C0*/	        0xC1,	0xC2,	0xC3,	0xC4,	0xC5,	0xC6,	0xC7,	0xC8,	0xC9,	0xCA,	0xCB,	0xCC,	0xCD,	0xCE,	0xCF,
	...
	INT8U SpecialCheck(INT8U ch)	// 공백 ! " # $ % & ' ( ) * + , - . / : ; < = > ? @ [ \ ' ^ _ ` { | } ~
	{
		if((ch >= 0x20 && ch <= 0x2f) || (ch >= 0x3a && ch <= 0x40) || (ch >= 0x5b && ch <= 0x60) || (ch >= 0x7b && ch <= 0x7e)) return ch;
		else return 0;
	}
