...
void slaughHouseNo_call_back(long num)
{
	...
	if(slaughHouseStr.index != num) {
		...
		memcpy((INT8U*)buffer, slaughHouseStr.slaughtHouse, 48);
		buffer[48]=0;
		Dsp_Write_String(disp_p,(HUGEDATA char *)buffer);
	}
}
...
//#endif
		...
		if (plu_check_inhibit_ptype(PTYPE_GROUP_NO))	// PLU 등급을 사용하지 않을 때
		{
			...
			caption_adjust(&cap,NULL);
			Menu_InputCreate(M_ID_TRS_GRADE_INDEX, (HUGEDATA char *)cap.form, MENU_TYPE_NUM, cap.input_x, cap.input_dlength, cap.input_length,
...
TRACE_MEATGRADE_STRUCT traceMeatGradeDefault[12] = {	
	{	1,	"1+",	2,	0},	// 1
	{	2,	"1",	3,	0},	// 2
	{	3,	"2",	4,	0},	// 3
	{	4,	"3",	5,	0},	// 4
...
#if defined(USE_EMART_MEATGRADE_PRINT) || defined(USE_MARBLING_GRADE_PRINT)
					TraceMeatGradeList_Len[type][i - 1] = strlen((char*)gradeText);
