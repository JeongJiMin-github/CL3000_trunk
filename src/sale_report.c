...
INT8U PrtGenWeightStr(char *dest, long weight, char weight_type, char *weight_sign1, char *weight_sign2, char decimal, char decimal_ch)
{
	...
	switch (weight_type) {
		...
		case 8: 					// " (6 lb 4 oz) 2835 g"  USA 전용 
			rate = 16;
			lb_val= weight/rate;
			oz_val= weight%rate;
		...
		case 9: 					// 198oz	// 12345g 
			...
			break;
		case 10:					// " (100 oz) 2835 g"
			weight = LbtoOz(weight, rate, decimal);
			rate= longmuldiv(weight, 283495L, 10000L);  //oz to g
			sprintf((char *)dest, " (%ld %s) %ldg", weight, weight_sign2, rate); // " (100 oz) 2835g"
			break;
		case 11:					// " 100oz (6 lb 4 oz)"
			...
			// kg -> lb
			lb_val = ad_cnv_weight_unit(weight,WEIGHT_UNIT_KG,WEIGHT_UNIT_LB);
			// lb -> oz
			rate = LbtoOz(lb_val, 16, decimal);
			
			lb_val = rate/16;
			oz_val = rate%16;
			sprintf((char *)dest, " %ldoz (%ld lb %ld oz)", rate, lb_val, oz_val); // 17oz (1 lb 1 oz)
			break;
#endif
	}
	return strlen((char *)dest);
...
//	flash_addr+= plu_get_ptype_point(PTYPE_PLU_NAME1, NULL, &name_size);		// Name
	...
	RcptCmd_SetFontStyle(3, RPT_FONT_TITLE, TKT_LAY_N, 0, LEFT);
#endif
...
//	RcptCmd_CharLF(1);
	...
	RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_N, 0, RIGHT);
	RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
	RcptCmd_SetFontStyle(3, RPT_FONT_SUBTITLE, TKT_LAY_N, 0, RIGHT);
	...
	RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_N, 0, LEFT);
	RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
	RcptCmd_SetFontStyle(3, RPT_FONT_SUBTITLE, TKT_LAY_N, 0, LEFT);
	receipt_caption_read(RCPTCAP_ID_REP_GRAND_TOTAL, string_buf);
	PutDataRxBuf(&CommBufPrt,string_buf,strlen(string_buf));
   	RcptCmd_CharLF(1);
...
INT8U DisplayPluSalesSummary(INT16S period, INT16U start_dept, INT16U end_dept, long start_no, long end_no, INT8U noprice, BASIC_REPORT_TYPE *br)
{
    ...
    disp_p = point(0,0);
    Dsp_Write_String(disp_p,(HUGEDATA char *)string_buf); // not reverse display
  #else
...
//				if(plu_table_search(psr->pstDept, psr->pstPLU) || psr->pstPLU >= MISC_PLU_WEIGHT) {
  ...
  #ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
    Dsp_Write_String(disp_p,(HUGEDATA char *)cap.form); // not reverse display
  #endif
#else
	memset((INT8U *)string_buf, ' ', 32);
	string_buf[32] = 0;
...
void DisplaySalesSummary(INT8U rep_mode, INT16S period, INT16U start_no, INT16U end_no, INT32U nv_addr, BASIC_REPORT_TYPE *br)
{
	...
	if(rep_mode == REPORT_CLERK && start_no == 1) {
		start_no = 0;
	}
...
READ_START:
	...
	if (GlbOper.realtimeTransactionType == 4 || GlbOper.realtimeTransactionType == 6)
	{
#ifdef USE_INDIA_FUNCTION
...
PAGE_VIEW:
  ...
  #ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
		disp_p = point(0,0);
		Dsp_Write_String(disp_p, "Remain");
  #endif
#else
#ifdef USE_ARAB_CONVERT
...
void TotalRcpt_PrintShopName(INT16U shop_no)
{
	...
	if(shop_no == store_str.code) {
		...
		if(store_str.text[0]) {
			//RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_N, 0, LEFT);
			RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_N, 0, CENTER);
...
#ifdef USE_ARAB_CONVERT
	RcptCmd_CharTab(TICKET_MARGIN);
	RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_0, 0, RIGHT);
	RcptCmd_PrintLine(1, 0);
#else
	RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_0, 0, LEFT);
...
void TotalRcpt_PrintVoid(void)
{
	...
	RcptCmd_TypeNone();
#ifdef USE_ARAB_CONVERT
	RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_1, 0, RIGHT);
#else
	RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_1, 0, LEFT);
#endif
	RcptCmd_PutStringInit(string_buf);
	RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
	// Print Void Count
...
//	dec_ch   =get_global_bparam(GLOBAL_DECIMAL_SIGN);
	...
	RcptCmd_SetFontStyle(3, RPT_FONT_SMALL, TKT_LAY_N, 0, LEFT);
...
// Born in Country
	RcptCmd_SetFontStyle(3, RPT_FONT_SMALL, TKT_LAY_2, 0, LEFT);
...
void TotalRcpt_PrintPayment(void)
{
	...
	RcptCmd_SetFontStyle(3, RPT_FONT_SUBTITLE, TKT_LAY_5, 0, LEFT);
#else
	RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_2, 0, LEFT);
#endif
...
void TotalRcpt_PrintChange(long changePrice)
{
	...
	RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
	RcptCmd_PutStringInit(string_buf);	
//SG	RcptCmd_SetFontStyle(3, RPT_FONT_SUBTITLE, TKT_LAY_1, 0, LEFT);
#ifndef	USE_AU_TICKET_DEFAULT
	RcptCmd_SetFontStyle(3, RPT_FONT_SUBTITLE, TKT_LAY_5, 0, LEFT);
#else
	RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_2, 0, LEFT);
...
void TotalRcpt_PrintGrandTotal(void)
{
	...
	char	currency_sign1[5];
#endif
	INT8U decimal, dec_ch, len;//, flag_tmp;
...
// Print Round Total Price
	...
	{
		RcptCmd_SetFontStyle(3, GlbFloat.printFontSize[2], TKT_LAY_5, 0, LEFT);	
...
void TotalRcpt_PrintCreditTotal(long totalPrice)
{
	...
	RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
#ifdef USE_ARAB_CONVERT
	RcptCmd_SetFontStyle(3, RPT_FONT_SUBTITLE, TKT_LAY_2, 0, RIGHT);
#else
	RcptCmd_SetFontStyle(3, RPT_FONT_SUBTITLE, TKT_LAY_2, 0, LEFT);
#endif
	RcptCmd_PutStringInit(string_buf);
...
void TotalRcpt_PrintTaxInfo(void)
{
	...
	if(printGSTsummary)
	{	
		RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_6, 0, LEFT);
		RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
		RcptCmd_PutStringInit(string_buf);
...
void TotalRcpt_PrintTaxInfo(void)
{
	...
	// Print TAX NO 
	RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_3, 0, LEFT);
	RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
	RcptCmd_PutStringInit(string_buf);
#ifdef USE_AU_TICKET_DEFAULT
	...
	for(i = 0; i < MAX_TAX_NO-1; i++) {
		...
		RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_4, 0, LEFT);
		...
		if(GlbOper.priceAddExTax) total -= PrtTotalStr.TaxTypePrice[i];
...
#ifdef USE_INDIA_FUNCTION
	RcptCmd_SetFontStyle(3, RPT_FONT_SMALL, TKT_LAY_N, 2, CENTER);
#else
	RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_N, 0, CENTER);
#endif
#ifdef USE_QR_ADDTOTAL
...
#ifdef USE_ARAB_CONVERT
	RcptCmd_SetFontStyle(3, GlbFloat.printFontSize[3] , TKT_LAY_6, 0, RIGHT);
#else
	RcptCmd_SetFontStyle(3, GlbFloat.printFontSize[3] , TKT_LAY_6, 0, LEFT);
#endif
	//RcptCmd_SetFontStyle(3, RPT_FONT_NORMAL, TKT_LAY_0, 0, LEFT);
...
INT8U PrintOneTransaction(TRANSACT_CHAIN  *cp, INT8U w_unit)
{
	...
	if(linked) {
#ifdef USE_ARAB_CONVERT
		RcptCmd_SetFontStyle(3, RPT_FONT_SMALL, TKT_LAY_N, 0, RIGHT);
#else
		RcptCmd_SetFontStyle(3, RPT_FONT_SMALL, TKT_LAY_N, 0, LEFT);
#endif
		RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
...
#ifdef USE_ARAB_CONVERT
	RcptCmd_SetFontStyle(3, GlbFloat.printFontSize[0], TKT_LAY_N, 0, RIGHT);
	RcptCmd_PrintMultiLine(string_buf, name_size, 1);
	RcptCmd_CharTab(1);
	RcptCmd_SetFontStyle(3, GlbFloat.printFontSize[1], TKT_LAY_0 , 0, RIGHT);
#else
	RcptCmd_SetFontStyle(3, GlbFloat.printFontSize[0], TKT_LAY_N, 0, LEFT);
	
	RcptCmd_CharTab(1);
	RcptCmd_PrintMultiLine(string_buf, name_size, 1);

	RcptCmd_SetFontStyle(3, GlbFloat.printFontSize[1], TKT_LAY_0 , 0, LEFT);
#endif
#ifdef USE_PLUNAME2_IN_TICKET
	if(plu_name2[0]!=0){
		RcptCmd_CharTab(TICKET_MARGIN+TICKET_TXT_MARGIN);
