...
void	label_message_call(long num)
{
	CAPTION_STRUCT cap;
	POINT disp_p;
	...
	plu_get_display_info(90,&cap,0);	// 90 PTYPE_LABEL_MSG_NO
...
{	
	...
	menu_displaytitle(0x1243, 0);	//0x1243 "DEL"
	menu_writepage(1, 1);

	caption_split_by_code(90, &cap, 0);	//90 "SMSG"(PLUStruct Identify)
  #ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
    disp_p = point(0,0);
...
REN1:
	keyin_setcallback(label_message_call);
	...
	{
		case KP_ENTER:
			addr  = PRT_ADVE_STRUCT_SIZE * (cur_id-1);
			addr += DFLASH_BASE + FLASH_PRT_ADVE_AREA;
			Flash_clear(addr, 0x00, PRT_ADVE_STRUCT_SIZE);
			break;
...
{
	SCROLL_MESSAGE_STRUCT scroll;
	char	readMsg[MAX_SCROLL_LINE][MAX_SCROLL_DATA+1];
	INT16S    i;
	INT32U  addr;
