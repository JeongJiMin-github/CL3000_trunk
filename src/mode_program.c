...
//	for (i=start_id; i<=end_id; i++) {
	...
	for (i=start_id; i<max_id; i++) {
		...
		if (caption_run_check(x,new_code)) {
			...
			data[34]=0;
  #ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
            caption_form_split_menu_graphic(data);
  #endif
#ifdef USE_SINGLE_LINE_GRAPHIC
...
#else  // defined(USE_USA_INGREDIENT) || defined(USE_NORTH_AMERICA_INGREDIENT)
