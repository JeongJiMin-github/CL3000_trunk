...
void	LoadGlobalSetting(void)
{
	...
	Disp_reset_flag = flag_tmp&0x0c; // param 737
	Disp_reset_flag = Disp_reset_flag>>2;
	...
	GlbOper.use_oz_fixedweight = OnOff(flag_tmp & (1<<5)); // parameter 754 Use oz in Fixed Weight Unit  1: Fixed Weight 의 단위를 oz로 사용 / 0: Fixed Weight 의 단위를 저울의 현 설정값을 따라가도록 설정
#endif
...
void mode_sale(void)
{
	...
	while (status_run.run_mode == RUN_SALE)
	{
        ...
        {// P679가 Y여도 Prepack, Save 키 누르면 PLU 해제 되지 않음.
			...
			{
				if((status_scale.Plu.ptype == PLU_BY_WEIGHT || status_scale.cur_keyvalue > 0) && ADM_GetData.Stable) // (Plu call 또는 Unit price 표시 값이 > 0) 이면서 stable
				{
		...
		sale_send_weight();
#endif
/*fdef USE_REALTIME_DATA_EVROOPT
        realtime_data_trans(); 
#endif*/
    } //while()
    CheckScroll();
...
INT16S LabelAndTransaction(void)
{
	...
	if(Operation.transAddup)
	{
		...
		if(GlbAddup.printLabel || Operation.operationClerk==OPER_STD_CLERK)	// no print , yes transaction
			...
			Operation.printON = OFF;
#ifdef USE_QR_ADDTOTAL
#ifndef USE_LABEL_PRINT_QRADD_SALE // QR Add 판매 시 개별 라벨 인쇄 사용
...
void sale_auto_check(void) // LP-II
{
	...
	//param 574 is Y (Allow Print Zero Total Price)인 경우 최소중량미만에서 AUTO 인쇄 안함.
	if (GlbOper.PrintZeroTotalPrice && (status_scale.weight_min > status_scale.Weight)) return;
	//param 783 is Y (Print Under Min.Weight on Auto)인 경우에 10d 미만에서 인쇄 안함(status_scale.weight_delta = 10d)
	if (GlbOper.PrintUnderMinWeightOnAUTO && (status_scale.weight_delta > status_scale.Weight)) return;
...
/*
#ifdef USE_SALE_MIN_WEIGHT_AT_AUTO_MODE
	if (ADM_GetData.Stable && (status_scale.weight_min <= status_scale.Weight || Operation.wAuto) && status_scale.Weight > 0)
#else
	if (ADM_GetData.Stable && status_scale.weight_min <= status_scale.Weight)
#endif //USE_SALE_MIN_WEIGHT_AT_AUTO_MODE
*/
	...
	//SG	else if( GlbHold.flagCalcOK && GlbHold.holdOperationTime &&(status_scale.weight_min <= status_scale.Weight) ) //SG070719. Hold Auto Check
	//SG	{
	//SG		if (CheckPluPrint(0))
	//SG		{
	//SG			PrintLabelWait();
	//SG			status_scale.print_multiple=1;
	//SG			StartPrintFlag = ON;
	//SG		}
	//SG	}
				...
				switch(KeyDrv.Type)
				{
					case KEY_TYPE_FUNCTION:
						switch (KeyDrv.CnvCode)
						{
							...
							case KS_PRINT:
							case KS_TARE:
								if (KeyDrv.IsConvCodeFlag) keyFlag = ON;
								else keyFlag = OFF;
								break;
						}
						break;
					...
					case KEY_TYPE_NUMERIC:
						...
						break; 
					case KEY_TYPE_SPEEDPLU:
						if ((KeyDrv.IsConvCodeFlag) && (KeyDrv.CnvCode == 0x0401)) keyFlag = ON;
						else keyFlag = OFF;
						break;
				}
			}
...
#endif //USE_REALTIME_DATA_EVROOPT		
...
INT16S GetDiscountData(INT16S id,DISCOUNT_STRUCT* pMem)
{
	...
	Flash_addr += DISCOUNT_STRUCT_SIZE * (id-1);//discount_str.code;
...
void sale_display_pluname(void)
{
	...
	if (status_scale.cur_pluid) {
		if(MISC_PLU_WEIGHT <= status_scale.cur_pluid)
		{
			...
			{
				memcpy(string_buf,status_scale.Plu.ItemName, 40);
				if (IsHanCharMid((char*)string_buf, 12) == CHAR_HAN_MID) { 		//오른쪽끝이 한글 중간일 경우 글씨 깨지는 현상 방지. 
					string_buf[11] = 0;
				}
			}
#endif	//USE_DIVIDING_FUNCTION
	...
	//	if(oldPrice != Price.TotalPrice[PRICE_RESULT])
	{
		...
		if (status_scale.cur_pluid && DiscSale.saleDiscountType)
		{
			...
			else
			{
				...
				FloatToString(TO_STR,priceBuf,9,DISPLAY_PRICE,&totalPrice,0,0);
				totalPrice = Price.TotalPrice[PRICE_ORIGIN];
				FloatToString(TO_STR,priceStr,9,DISPLAY_PRICE,&totalPrice,0,0);
				totalPrice = Price.TotalPrice[PRICE_ORIGIN] - Price.TotalPrice[PRICE_SAVED]; 
				FloatToString(TO_STR,dc_price,9,DISPLAY_PRICE,&totalPrice,0,0);
				
				if (DiscSale.discIndexData.discPriceType == DISC_PRICE_FIXED && !Operation.fixedDiscFlag)
				{
					FloatToString(TO_STR,priceBuf,9,DISPLAY_PRICE,&Price.FixedDiscountPrice,0,0);
...
#if defined(USE_FIXEDWEIGHT_UNIT_OZ) // Only Count, PCS PLU with Fixed Weight or actual Weight
...
void sale_display_ClerkSubtotalList (void)
{
	...
	if (status_scale.cur_displayon&chk)
	{
		...
		if(Clerk_Subtotal_List == 1)
		{
			...
			{
				FloatToString(TO_STR,priceBuffer1,8,DISPLAY_PRICE,&totalData.TotalPrice,0,0);	  
				...
				for(i=viewList;i<maxIndex ;i++)
				{
					if(i<maxIndex)
					{
						...
						FloatToString(TO_STR,priceBuf,8,DISPLAY_PRICE,&temptrtvolume,0,0);  //se-hyung 20071017
...
void sale_display_proc(INT8U onlyWeighing)
{
	...
	if (status_scale.flag_display&0x01) {
		...
		sale_display_total(onlyWeighing);
