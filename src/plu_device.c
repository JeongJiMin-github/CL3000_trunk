...
//		sum+=plu_struct_set(12,"Prefix"			,PTYPE_PREFIX			,'S', 0l,      0l, 2, 1, 0,i);
		...
		sum+=plu_struct_set(idx++,"Netweight Dec"	,PTYPE_NETWEIGHT_DECIMAL,'B', 0l,		4l, 1, 1, 0,i);
...
void plu_struct_set_base(void)
{
	...
	#ifdef USE_EXPANDED_MAX_BARCODE_99
		sum+=plu_struct_set(idx++,"Barcode No."		,PTYPE_BARCODE_NO			,'W', 0l,     99l, 2, 1, 0,i);	// 25
	#else
		sum+=plu_struct_set(idx++,"Barcode No."		,PTYPE_BARCODE_NO			,'W', 0l,     20l, 2, 1, 0,i);	// 25
	#endif	
		sum+=plu_struct_set(idx++,"Aux BCode No."	,PTYPE_AUX_BARCODE_NO		,'W', 0l,     20l, 2, 1, 0,i);	// 26
...
//	if(flag==1) { // Move 
		...
		if(dc_no) {
			addr_target = dc_no-1;
			addr_target *= DISCOUNT_STRUCT_SIZE;
			addr_target += FLASH_DISCOUNT_AREA;
			addr_target += DFLASH_BASE;
