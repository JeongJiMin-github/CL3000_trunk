...
void	groupText_call(long num)
{
	...
	plu_get_display_info(PTYPE_GROUP_NO, &cap, 0);
...
void    group_delete(void)		// command code  0x1223
{
	...
	menu_displaytitle(0x1223,0);	//0x1223 "DEL"
	menu_writepage(1, 1);

	caption_split_by_code(9, &cap,0);	//9 "GROUP"(PLUStruct Identify)
  #ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
    disp_p = point(0,0);
...
REN1:
	...
	keyin_setcallback(groupText_call);
	...
	{
		case KP_ENTER:
			addr  = GROUP_STRUCT_SIZE * (cur_id-1);
			addr += DFLASH_BASE + FLASH_SUB_DEPARTMENT_AREA;
			Flash_clear(addr, 0x00, GROUP_STRUCT_SIZE);
			break;
