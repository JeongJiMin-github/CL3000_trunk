...
#endif // USE_INGREDIENT_98_PROTOCOL
#endif // USE_INGREDIENT_98_PROTOCOL
...
END2:
	if( SearchParamSet(fcode, &pset) ){
        ...
        if (fcode == 1856) 
        {
            remote_manager_permission_edit();
            input_data = get_global_bparam(GLOBAL_SALE_SETUP_FLAG19) & 0x04;
            input_data = input_data>>2;
            if(input_data)set_nvram_bparam(NVRAM_SELF_KEY_BLOCK_ON, 1);
            else set_nvram_bparam(NVRAM_SELF_KEY_BLOCK_ON, 0);
        }
#endif
        ret=1;
	}
