...
#else
        if (keyRawData == 0xFF) // key released
        {
            keyValidRawData = 0xFF;
            keyDrvPressCount = 0;
        }
		...
		if (PS2_IsKey()) {
...
#endif
			{
				if(KeyDrv.FirstKey) 
				{
					...
					memset(Scanner.barcodeData, 0, SCANNER_BARCODE_DATA_LEN);
				}
				...
				if(KeyDrv.CnvCode >= KEY_NUM_0 && KeyDrv.CnvCode <= KEY_NUM_10) 
				{
					...
					if (Scanner.barcodeIndex >= SCANNER_BARCODE_DATA_LEN) Scanner.barcodeIndex = SCANNER_BARCODE_DATA_LEN - 1;	// buffer overflow 방지
					goto REPT;
				}
