...
long	GetTicketCounter(void)
{
	...
	if(ReadSystemNewDateTicket() || (count < 1L) || (count > 99999L)) //se-hyung 20070831
	{
		count = 1L;
...
{   
	...
	nvram_set(NVRAM_TRANS_DATA, 0, MAX_TRANSACTION_ADD_SIZE*MAX_ADD_TRANSACTION);
...
#ifndef USE_TRACE_STANDALONE	// 개체기억기능 영역공유
	if (ethernet_list.status != 2) SetClerkReopenTransaction(ClerkNum, ReopenedTransPay);
...
// decFlag(0~1) : 7-Segment 디스플레이 출력시 dot 길이 제외(1), 그 외의 경우 dot 포함(0)
