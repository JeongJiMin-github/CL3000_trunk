...
void caption_form_split_menu_graphic(char *caption_form);
#endif
...
#endif
#ifndef USE_QR_ADDTOTAL
	if(code == 0x1264) return 0;
#endif
...
#ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
	caption_form_split_menu_graphic(str);
#endif
	return 1;
}
...
void caption_form_split_menu_graphic(char *caption_form)
{
    ...
    memset(form_graphic, 0x00, 12);

    for(i = 0; i < CAP_FORM_LENGTH; i++)
    {
        if (caption_form[i] == '|') break;
    }

    if (i == CAP_FORM_LENGTH)
        strncpy(form_graphic, caption_form, 12);
    else
        strncpy(form_graphic, &caption_form[i+1], 12);
    caption_form[i] = 0;
}
#endif
...
void caption_form_check(CAPTION_STRUCT *caption,INT8U mode)
{
	INT8U i;
#ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
    caption_form_split_menu_graphic(caption->form);
#endif		
