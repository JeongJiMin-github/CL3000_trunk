...
INT8U menuDispOn;
INT16U prevDispTimer100ms;
INT8U MenuStr[MAX_GRAPHIC_STR];
INT8U SubMenuStr[MAX_MENU_INDEX][MAX_GRAPHIC_STR];
#endif
...
#ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
    if (form_graphic) strcpy(&(SubMenuStr[MenuIndex][0]), (const char *)form_graphic);
    else memset(SubMenuStr[MenuIndex], 0, MAX_GRAPHIC_STR);
#endif
	MenuIndex++;
}
...
{
#if !defined(USE_SINGLE_LINE_GRAPHIC) || defined(USE_MENU_DISPLAY_ON_GRAPHIC_AREA)
	POINT disp_p;
#endif
...
//	start_line=1;
  ...
  #ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
			if (status_run.run_mode == RUN_PROGRAM && !MenuRefresh)
			{
				if (!menuDispOn)
				{
					disp_p = point(0,0);
					DspStruct.DirectDraw = 1;
					Dsp_Write_String(disp_p, &SubMenuStr[view_idx][0]);
					DspStruct.DirectDraw = 0;
					prevDispTimer100ms = SysTimer100ms;
					if(Menu[view_idx].function != NULL) menuDispOn = ON;
				}
			}
  #endif
	...
	#ifdef USE_LABEL_NAME_SCROLL
   							if(Label_name_Scroll_flag == ON)	//M1510 라벨명 스크롤 기능
   							{
   								keyin_setanycheck(plucall_label_id_scroll);
   							}
	#endif
  #endif
...
#ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
                                    if (!menuDispOn) // display after error
                                    {
                                        DspStruct.DirectDraw = 1;
                                        Dsp_Write_String(disp_p, &SubMenuStr[view_idx][0]);
                                        DspStruct.DirectDraw = 0;
...
#ifndef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
  #ifdef USE_LABEL_NAME_SCROLL
						if(Label_name_Scroll_flag == ON)
							Label_name_Scroll_flag = OFF;
  #endif
#endif
						break;
					case MENU_TYPE_NUM_1BYTE :
...
*/						}
				...
				MenuRefresh = OFF;
#ifdef USE_MENU_DISPLAY_ON_GRAPHIC_AREA
				if (!menuDispOn) // End Function -> MenuRefresh 후 초기화 된 화면에 메뉴 문구 표시 
				{
					DspStruct.DirectDraw = 1;
					Dsp_Write_String(disp_p, &SubMenuStr[view_idx][0]);
					DspStruct.DirectDraw = 0;
