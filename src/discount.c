...
FUNCTION_IN:
	...
	if(dc_no) {
		addr = dc_no-1;
		addr *= DISCOUNT_STRUCT_SIZE;
		addr += FLASH_DISCOUNT_AREA;
		addr += DFLASH_BASE;
...
void discount_delete_byid(INT16S dc_no)
{
	...
	addr = dc_no - 1;
	addr*= DISCOUNT_STRUCT_SIZE;
	addr+=  DFLASH_BASE + FLASH_DISCOUNT_AREA;
...
READ_START:
	...
	for (i = 0; i < line_ct; i++)
	{
		...
		addr_flash = addr_origin + DISCOUNT_STRUCT_SIZE * data_pos[cur_id+i];
...
INT8U discount_check_dc_number(INT16U dc_no, INT16U *dept_no, INT32U *plu_no)
{
	...
	flash_addr = DFLASH_BASE + FLASH_DISCOUNT_AREA;
	flash_addr += DISCOUNT_STRUCT_SIZE*(dc_no-1);
	*dept_no = Flash_wread(flash_addr);
...
INT32U	get_dc_last_plu_no(INT16U dept_no)
{
	...
	INT32U 	addr_origin, flash_addr, plu_no=0, temp_no;
	INT16U	dc_no;
...
INT8U	get_dc_any_plu_no(INT16U dept_no)
{
	...
	addr_origin =  DFLASH_BASE + FLASH_DISCOUNT_AREA;
   	for(dc_no=0; dc_no<MAX_DISCOUNT_NO; dc_no++) {
		flash_addr = addr_origin + (INT32U)DISCOUNT_STRUCT_SIZE*(INT32U)dc_no;
...
DISCOUNT_START:
	...
	flash_addr = addr_origin + DISCOUNT_STRUCT_SIZE*(dc_no-1);
	if (dc_no>=MAX_DISCOUNT_NO) 
	{	// data full
		error_display_caption(0xF035);
		Flash_sread(addr_origin, (HUGEDATA INT8U*)&disc_str, DISCOUNT_STRUCT_SIZE); 
	} 
	else 
	{
...
INT16S input_discount_data(DISCOUNT_STRUCT *dc_str, INT8U plu_type, INT8U start_page)
{
	...
	char 	fdaytype_str[15],timetype_str[15];
